...
snaps, snap_strings = filter_snapshots(datadir, 304, 466, sampling=1, str_snaps=True)
...
lims = {
    ...
    ("ramses", "Metallicity"): (1e-5 * zsun, 5 * zsun),
}
...
for i, sn in enumerate(snaps):
    ...
    # %%
    fig, ax = plt.subplots(3, 1, figsize=(5, 10), dpi=300, sharex=True, sharey=True)
    ...
    # multiphase
    ax[0].axhline(2, lw=1, ls="--", color="grey", alpha=0.8)
    ax[0].axhline(np.log10(5e4), lw=1, ls="--", color="grey", alpha=0.8)
    ...
    # bar .ax[0].xaxis.set_tick_params(pad=2)
    bar.set_label(r"$\mathrm{\log\:Gas\:Metallicity\: \left[Z_{\odot} \right]}$")
    bar.ax.xaxis.set_label_position("top")
    bar.ax.xaxis.set_ticks_position("top")
